...
def join_timestamps(comment_dict):
    ...
    all_timestamps_counter = [
        i[0]
        for i in sorted(
            all_timestamps_counter.items(), key=lambda k: k[1], reverse=True
        )
    ]
    ...
    for k in all_timestamps_counter:
        if (
            ...
            or k - 2 in merged_timestamp_list
            or k - 1 in merged_timestamp_list
            or k + 1 in merged_timestamp_list
            or k + 2 in merged_timestamp_list
            or k + 3 in merged_timestamp_list
        ):
            continue
        else:
    ...
    d = sorted(
        processed_timestamp_comment.items(), key=lambda x: len(x[1]), reverse=True
    )
...
def create_word_frequency_table(comment_list):
    ...
    sorted_dict = {
        k: v
        for k, v in sorted(freqTable.items(), key=lambda item: item[1], reverse=True)
    }
    count = 0
    keywords = []
...
def get_sentiment_data(f_list):
    ...
    for fname in f_list:
        with open(fname, "r") as f:
            reader = csv.reader(f)
            for row in reader:
                timestamp, comment, like_cnt, sentiment_score = row
                all_scores.append(sentiment_score)

                score_dict[round(float(sentiment_score), 1)].append(comment)

    score_dict = OrderedDict(sorted(score_dict.items(), reverse=True))

    return all_scores, score_dict
